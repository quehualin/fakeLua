...
typedef struct lua_State
{
    ...
    int status;                 // lua state的状态
    struct lua_State *next;     // 下一个lua_State，通常创建协程时会产生
    struct lua_State *previous; // 上一个lua_State，通常创建协程时会产生
...
{
    struct lua_State *mainthread; // 我们的lua_State其实是lua_thread，某种程序上来说，它也是协和
    lua_Alloc frealloc;           // 一个可以自定义的内存分配函数
    void *ud;                     // 当我们自定义内存分配器时，可能要乃至这个结构，但是我们用官方默认的版本
                                  // 因此它始终是NULL
    lua_CFunction painc;          // 当调用LUA_THROW接口时，如果当前不处于保护模式，那么会直接调用panic函数
                                  // panic函数通常是输出一些关键日志
} global_State;
